...
func TestNewXReader(t *testing.T) {
	f, err := os.Open("examples/a.lzma")
	if err != nil {
		t.Fatalf("open examples/a.lzma: %s", err)
	}
	defer f.Close()
	_, err = NewXReader(bufio.NewReader(f))
	if err != nil {
...
func xreadOrigFile(t *testing.T) []byte {
...
func testXDecodeFile(t *testing.T, filename string, orig []byte) {
	...
	l, err := NewXReader(bufio.NewReader(f))
	if err != nil {
		t.Fatalf("NewReader: %s", err)
	}
...
func TestXReaderSimple(t *testing.T) {
	// DebugOn(os.Stderr)
	// defer DebugOff()
...
func TestXReaderAll(t *testing.T) {
	dirname := "examples"
...
type xwrapTest struct {
	name string
	wrap func(io.Reader) io.Reader
}

func (w *xwrapTest) testFile(t *testing.T, filename string, orig []byte) {
	pathname := filepath.Join(dirname, filename)
	f, err := os.Open(pathname)
	if err != nil {
...
func TestXReaderWrap(t *testing.T) {
	tests := [...]xwrapTest{
...
func TestXReaderBadFiles(t *testing.T) {
	dirname := "examples"
	dir, err := os.Open(dirname)
	if err != nil {
		t.Fatalf("Open: %s", err)
...
type xrepReader byte

func (r xrepReader) Read(p []byte) (n int, err error) {
	for i := range p {
		p[i] = byte(r)
	}
	return len(p), nil
...
func newxRepReader(c byte, n int64) *io.LimitedReader {
	return &io.LimitedReader{R: repReader(c), N: n}
}

func newXCodeReader(r io.Reader) *io.PipeReader {
	pr, pw := io.Pipe()
	go func() {
...
func TestXReaderErrAgain(t *testing.T) {
