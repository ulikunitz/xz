...
type withWindowSize struct {
	size int
}

func (w withWindowSize) updateWriterConfig(cfg *writerConfig) error {
	if w.size < 0 {
		return errors.New("xz: writer window size must be non-negative")
	}
	cfg.WindowSize = w.size
	return nil
}

func WithWindowSize(size int) WriterOption {
	return withWindowSize{size: size}
}

type withBufferSize struct {
	size int
}

func (w withBufferSize) updateWriterConfig(cfg *writerConfig) error {
	if w.size < 0 {
		return errors.New("xz: writer buffer size must be non-negative")
	}
	cfg.BufferSize = w.size
	return nil
}

func WithBufferSize(size int) WriterOption {
	return withBufferSize{size: size}
}
...
func (w withProperties) updateWriterConfig(cfg *writerConfig) error {
	cfg.Properties = w.properties
	return nil
}

func WithProperties(properties lzma.Properties) WriterOption {
	return withProperties{properties: properties}
}

type withPathFinder struct {
	pathFinder string
}

func (w withPathFinder) updateWriterConfig(cfg *writerConfig) error {
	if w.pathFinder == "" {
		return errors.New("xz: writer path finder must be non-empty")
	}
	cfg.PathFinder = w.pathFinder
	return nil
}

func WithPathFinder(pathFinder string) WriterOption {
	return withPathFinder{pathFinder: pathFinder}
}
...
func (w withMapper) updateWriterConfig(cfg *writerConfig) error {
	if w.mapper == "" {
		return errors.New("xz: writer mapper must be non-empty")
	}
	cfg.Mapper = w.mapper
	return nil
}
...
func WithMapper(mapper string) WriterOption {
	return withMapper{mapper: mapper}
}

type withMinMatchLen struct {
	minMatchLen int
}

func (w withMinMatchLen) updateWriterConfig(cfg *writerConfig) error {
	if w.minMatchLen < 2 {
		return errors.New("xz: writer minimum match length must be at least 2")
	}
	cfg.MinMatchLen = w.minMatchLen
	return nil
}

func WithMinMatchLen(minMatchLen int) WriterOption {
	return withMinMatchLen{minMatchLen: minMatchLen}
}

type withMaxMatchLen struct {
	maxMatchLen int
}

func (w withMaxMatchLen) updateWriterConfig(cfg *writerConfig) error {
	if w.maxMatchLen < 2 {
		return errors.New("xz: writer maximum match length must be at least 2")
	}
	cfg.MaxMatchLen = w.maxMatchLen
	return nil
}

func WithMaxMatchLen(maxMatchLen int) WriterOption {
	return withMaxMatchLen{maxMatchLen: maxMatchLen}
}
...
func (w withChecksum) updateWriterConfig(cfg *writerConfig) error {
	if err := verifyFlags(w.checksum); err != nil {
		return err
	}
	cfg.Checksum = w.checksum
	return nil
}

func WithChecksum(checksum byte) WriterOption {
	return withChecksum{checksum: checksum}
}
...
// NewWriter creates a new Writer for xz-compressed data.
func NewWriter(xz io.Writer, options ...WriterOption) (w WriteFlushCloser, err error) {
	cfg := writerConfig{
		WindowSize: 1 << 23,
		BufferSize: 1 << 23,
