...
func (w withWorkers) updateReaderConfig(cfg *readerConfig) error {
	if w.workers < 1 {
...
func (w withWorkers) updateWriterConfig(cfg *writerConfig) error {
	if w.workers < 1 {
		return errors.New("xz: writer workers must be >= 1")
	}
	cfg.Workers = w.workers
	return nil
}

func WithWorkers(workers int) RWOption {
	return &withWorkers{workers: workers}
}

type withSingleStream struct {
	flag bool
}

func (w withSingleStream) updateReaderConfig(cfg *readerConfig) error {
	cfg.SingleStream = w.flag
	return nil
}

func WithSingleStream() ReaderOption {
	return &withSingleStream{flag: true}
}

type withLZMAParallel struct {
	flag bool
}

func (w withLZMAParallel) updateReaderConfig(cfg *readerConfig) error {
	cfg.LZMAParallel = true
	return nil
}

func (w withLZMAParallel) updateWriterConfig(cfg *writerConfig) error {
	cfg.LZMAParallel = true
	return nil
}

func WithLZMAParallel() RWOption {
	return &withLZMAParallel{flag: true}
}

type withLZMABufferSize struct {
	size int
}

func (w withLZMABufferSize) updateReaderConfig(cfg *readerConfig) error {
	if w.size <= 0 {
		return errors.New("xz: reader LZMA buffer size must be > 0")
	}
	cfg.LZMABufferSize = w.size
	return nil
}

func WithLZMABufferSize(size int) ReaderOption {
	return &withLZMABufferSize{size: size}
}
...
// NewReader creates an io.ReadCloser.
func NewReader(xz io.Reader, options ...ReaderOption) (r io.ReadCloser, err error) {
	cfg := readerConfig{Workers: runtime.GOMAXPROCS(0)}
	for _, o := range options {
		if err = o.updateReaderConfig(&cfg); err != nil {
			return nil, err
		}
	}

	if cfg.LZMAParallel && cfg.LZMABufferSize == 0 {
		cfg.LZMABufferSize = 8 << 20
	}
...
func newMultiThreadStreamReader(xz io.Reader, options *readerConfig) streamReader {
