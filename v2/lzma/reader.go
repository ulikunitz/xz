...
// ReaderOptions stores the parameters for the reader of the classic LZMA
// format.
...
//
//   - The [ReaderOptions] DictCap field is now interpreted as a limit for the
//     dictionary size.
//   - The default is 2 Gigabytes (2^31 bytes).
//   - Users can check with the [Reader.Header] method what the actual values are in
//     their LZMA files and set a smaller limit using [ReaderOptions].
...
func (r *Reader) init(z io.Reader, hdr Header) error {
	...
	if err := r.lzDecoder.Init(lz.DecoderOptions{WindowSize: int(hdr.DictSize)}); err != nil {
		return err
	}
