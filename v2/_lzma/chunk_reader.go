...
// the dictSize to support a linear.lzDecoder or 2 MiB.
func (r *chunkReader) init(z io.Reader, dictSize int) error {
	*r = chunkReader{r: z}
	do := lz.DecoderOptions{
		WindowSize: dictSize,
		BufferSize: 2 * dictSize,
	}
	if do.BufferSize < maxUncompressedChunkSize {
		do.BufferSize = maxUncompressedChunkSize
	}
	err := r.lzDecoder.Init(do)
	return err
}
...
func (r *chunkReader) readChunk() error {
	...
	if h.Control == CU || h.Control == CUD {
		...
		_, err = io.CopyN(&r.lzDecoder, r.r, int64(h.Size))
		if err == io.EOF {
			err = io.ErrUnexpectedEOF
		}
		return err
